...
parser.add_argument(
    "result_file",
    nargs="?",
    default="/tmp/mquery-last-result.json",
    help="output file",
)
    ...
    for r in results:
        ...
        meta = ", ".join(
            filter(
                ...
                list(
                    map(
                        lambda o: r["metadata"][o].get("display_text"),
                        r["metadata"].keys(),
                    )
                ),
            )
...
res = requests.post(
    MQUERY_SERVER + "/query",
    json={"method": "query", "raw_yara": yara_rule.decode("utf-8")},
    verify=MQUERY_SSL_VERIFY,
)
...
with tqdm(total=0) as pbar:
    while not out or out["job"]["status"] not in ["cancelled", "failed"]:
        ...
        res = requests.get(
            MQUERY_SERVER + "/status/{}".format(query_hash),
            verify=MQUERY_SSL_VERIFY,
        )
        out = res.json()
        ...
        if out["job"]["status"] == "done":
            tagged = sum(
                map(
                    lambda m: 1 if m["metadata_available"] else 0,
                    out["matches"],
                )
            )
