...
class YaraParser(object):
    ...
    def string_to_query(self, value):
        ...
        if value[0] == "{" and value[-1] == "}":
            ...
            inner_val = (
                inner_val.decode("ascii").replace("\\\\", "\\").replace("\\'", "'")
            )
            return '"' + inner_val + '"'
        else:
            assert False
    ...
    def get_grammar(self):
        ...
        ).setParseAction(self.act_variable)
        variable_list = Or(
            [
                Literal("(").suppress()
                + Group(
                    variable + ZeroOrMore(Literal(",").suppress() + variable)
                ).setParseAction(self.act_list)
                + Literal(")").suppress(),
                Literal("them").setParseAction(self.act_them),
            ]
        )
        count_specifier = Or([Literal("any"), Literal("all"), Word("0123456789")])
        bracketed_atom = Literal("(").suppress() + expression + Literal(")").suppress()
        ignoreme_expr = (variable + "==" + Word(string.digits)).setParseAction(
            self.act_ignore
        )
        atom << Or(
            [
                ...
                ignoreme_expr,
                bracketed_atom,
                (count_specifier + "of" + variable_list).setParseAction(
                    self.act_multiselector
                ),
            ]
        )
        expression << (
            atom + Optional(Or(["and" + expression, "or" + expression]))
