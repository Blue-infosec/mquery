...
def job_daemon():
    ...
    while True:
        queue, data = redis.blpop(
            ["queue-search", "queue-index", "queue-metadata", "queue-yara"]
        )
            ...
            if dep not in current_meta:
                raise RuntimeError(
                    (
                        ...
                        "{} depends on {} but is declared earlier in config."
                    ).format(extr_name, dep)
                )

            # we build local dictionary for each extractor
            # thus enforcing dependencies to be declared correctly
